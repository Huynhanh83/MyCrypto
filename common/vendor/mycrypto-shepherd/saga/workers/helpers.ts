...
function* sendRequestToProvider(providerId: string, rpcMethod: AllProviderMethods, rpcArgs: any) {
  try {
    const {
      provider,
      timeoutThreshold
...
function* processRequest(providerId: string, workerId: string) {
  ...
  const { payload }: IProviderCallRequested = yield apply(providerChannels, providerChannels.take, [
    providerId
  ]);
