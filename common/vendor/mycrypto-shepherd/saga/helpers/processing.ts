 ...
 */
export function* processProvider(providerId: string, { concurrency }: IProviderConfig) {
  const timer = trackTime();
  const providerIsOnline: boolean = yield call(checkProviderConnectivity, providerId);
