...
function* getOptimalProviderId(payload: IProviderCallRequested['payload']): SagaIterator {
...
function* handleRequest(): SagaIterator {
  ...
  while (true) {
    ...
    const action: IProviderCallRequested = yield apply(balancerChannel, balancerChannel.take);
    ...
    function* process() {
      ...
      const providerId: string | undefined = yield call(getOptimalProviderId, payload);

      if (providerId) {
        yield apply(providerChannels, providerChannels.put, [providerId, action]);
      }
    }

    const queueTimeoutMs: ReturnType<typeof getQueueTimeout> = yield select(getQueueTimeout);
