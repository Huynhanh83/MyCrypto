...
export abstract class BaseChannel {
  ...
  public *take(): SagaIterator {
    ...
    yield put(
      subscribeToAction({
        ...
        callback: () => {
          this.log(`call ${this.currentAction && this.currentAction.payload.callId} is now null `);
  ...
  public *cancelPendingCalls(): SagaIterator {
    const pendingCalls: IProviderCallRequested[] = yield apply(this, this.getPendingCalls);
  ...
  private *getPendingCalls(): SagaIterator {
    ...
    const pendingCalls = this.currentAction ? [...queuedCalls, this.currentAction] : queuedCalls;
