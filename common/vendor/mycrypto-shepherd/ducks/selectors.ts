...
} from './providerBalancer/providerCalls';
...
export const getAllProvidersOfNetwork = (state: RootState, networkId: string) => {
  ...
  for (const providerId of availableProviderIds) {
    ...
    Object.entries(providerConfig.supportedMethods).forEach(
      ([rpcMethod, isSupported]: [AllProviderMethods, boolean]) => {
        availableMethods[rpcMethod] = availableMethods[rpcMethod] || isSupported;
      }
    );
  }
...
export const getAvailableProviderId = (state: RootState, payload: IProviderCall) => {
  ...
  for (const currentProviderId of listToPrioritizeByWorker) {
    ...
    if (
      !prevProvider ||
      prevProvider.numOfRequestsCurrentProcessing > numOfRequestsCurrentProcessing
    ) {
      prevProvider = {
