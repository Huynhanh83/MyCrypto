...
} from '../providerBalancer/balancerConfig';
...
export const triggerOnMatchingCallId = (callId: number, includeTimeouts: boolean) => (
  action: AllActions
) => {
...
export function waitForNetworkSwitch(dispatch: Dispatch<RootState>, id: number) {
...
export function waitForManualMode(dispatch: Dispatch<RootState>): Promise<string> {
  function triggerOnSuccessOrFail(action: AllActions) {
    return action.type === BALANCER.MANUAL_SUCCEEDED || action.type === BALANCER.MANUAL_FAILED;
  }
