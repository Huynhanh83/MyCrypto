...
const handleWorkerKilled: WReducer = (state: IWorkerState, { payload }: IWorkerKilled) => {
...
const handleWorkerSpawned: WReducer = (state: IWorkerState, { payload }: IWorkerSpawned) => {
  if (state[payload.workerId]) {
...
const handleProviderAdded: WReducer = (state, { payload }: IProviderStatsAdded) => {
  const stateCopy = { ...state };
  for (const [workerId, worker] of Object.entries(payload.workers)) {
...
const handleProviderCallSucceeded: WReducer = (
  ...
  const worker = Object.entries(state).find(
    ([_, { currentPayload }]) => !!(currentPayload && currentPayload.callId === callId)
  );
...
const handleProviderCallTimeout: WReducer = (
  ...
  const worker = Object.entries(state).find(
    ([_, { currentPayload }]) => !!(currentPayload && currentPayload.callId === providerCall.callId)
  );
...
export const workerReducer: WReducer = (
  ...
  action: WorkerAction | ProviderCallAction | BalancerAction | ProviderStatsAction
): IWorkerState => {
  switch (action.type) {
    case WORKER.SPAWNED:
