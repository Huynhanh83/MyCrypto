...
type NReducer<T> = (state: IProviderStatsState, action: T) => IProviderStatsState;
...
const handleWorkerKilled: NReducer<IWorkerKilled> = (
  ...
  const nextProviderProviderStatsState = {
    ...providerToChange,
    currWorkersById: providerToChange.currWorkersById.filter(id => id !== workerId)
  };
...
const handleProviderRemoved: NReducer<IProviderStatsRemoved> = (state, { payload }) => {
