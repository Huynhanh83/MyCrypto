...
export class Web3Client extends RPCClient {
  ...
  public batch = (requests: RPCRequest[] | any): Promise<IJsonRpcResponse[]> =>
    this.sendAsync(requests.map(this.decorateRequest)) as Promise<IJsonRpcResponse[]>;

  private readonly sendAsync = (request: any): Promise<IJsonRpcResponse | IJsonRpcResponse[]> => {
    return new Promise((resolve, reject) => {
      this.provider.sendAsync(request, (error, result: IJsonRpcResponse | IJsonRpcResponse[]) => {
        if (error) {
          return reject(error);
        }
        resolve(result);
